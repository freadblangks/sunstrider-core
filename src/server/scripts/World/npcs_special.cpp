                    ...
                    switch (_nextAction)
                    {
                        case EVENT_DO_JUMP:
                            if (GameObject* target = VerifyTarget())
								me->GetMotionMaster()->MoveCharge(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), 42.0f, MOVEID_JUMP, true);
                            _nextAction = 0;
                            break;
    ...
    struct npc_argent_squire_gruntlingAI : public ScriptedAI
    {
        ...
        void ScheduleTasks()
        {
            _scheduler
                ...
                .Schedule(Seconds(1), [this](TaskContext context)
                {
                    if ((me->HasAura(SPELL_AURA_TIRED_S) || me->HasAura(SPELL_AURA_TIRED_G)) && me->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_BANKER | UNIT_NPC_FLAG_VENDOR))
...
public:
    ...
    struct npc_bountiful_tableAI : public PassiveAI
    {
        ...
        void PassengerBoarded(Unit* who, int8 seatId, bool /*apply*/) override
        {
            ...
            init.SetFacing(o);
            who->GetMotionMaster()->LaunchMoveSpline(std::move(init), 0, MOTION_PRIORITY_HIGHEST);
